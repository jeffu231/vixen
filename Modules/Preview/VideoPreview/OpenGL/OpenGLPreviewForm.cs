...
namespace VixenModules.Preview.VideoPreview.OpenGL
{
	...
	public partial class OpenGlPreviewForm : Form, IDisplayForm
	{
		...
		public OpenGlPreviewForm(VideoPreviewData data, Guid instanceId)
		{
			_formLoading = true;
		...
		private void HandleContextMenu()
		{
			...
			//item.Enabled = _videoImage.HasBackground;
			//item.Click += (sender, args) =>
			//{
			//	_width = _videoImage.Width;
			//	_height = _videoImage.Height;
		...
		private void Initialize()
		{
			...
			_videoImage = new VideoImage(_width, _height);
		...
		private void OnRenderFrame()
		{
			...
			if (_videoElement.State?.Count > 0)
			{
				...
				lock (ContextLock)
				{
					...
					_sw2.Restart();
					UpdateBackground();
					_videoImage.Draw(perspective, _camera.ViewMatrix);
		...
		private void UpdateBackground()
		{
			...
			if (image != null)
			{
				_videoImage.Content = new Bitmap(image);			}
		}
		...
		private void CreateCamera()
		{
			...
			var name = $"OpenGLPreview_{InstanceId}";
			// create our camera
			_camera = new Camera(new Vector3(Width / 2f, Height / 2f, _focalDepth), Quaternion.Identity);
			_camera.SetDirection(new Vector3(0, 0, -1));
